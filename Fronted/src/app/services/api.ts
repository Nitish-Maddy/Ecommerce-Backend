...
function resolvePaymentBase(): string {
  ...
  if (explicit) {
    const b = explicit.replace(/\/$/, '');
    return b.endsWith('/api/v1') ? `${b}/payments` : `${b}/api/v1/payments`;
  }
  if (import.meta.env.DEV) {
    return '/api/v1/payments';
  }
  return 'http://localhost:5000/api/v1/payments';
}
...
/** Clear stored auth and notify AuthContext so UI updates immediately. */
...
async function request<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (!res.ok) {
    // Auto-logout on 401 so stale / expired tokens don't keep failing
    if (res.status === 401) {
      handleAuthExpired();
    }
